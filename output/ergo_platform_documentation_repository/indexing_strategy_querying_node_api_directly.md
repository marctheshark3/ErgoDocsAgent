...
Source: docs/dev/tutorials/blockchain-indexing/node-api-direct.md
...
Another approach to accessing blockchain data is to run your own Ergo node and query its [REST API](swagger.md) directly from your application. ## Concept

Instead of relying on a third-party explorer, your application communicates directly with an Ergo node instance that you control. This node maintains a copy of the blockchain (either full or pruned) and exposes endpoints to retrieve block, transaction, and state information. ## How It Works

1. **Run an Ergo Node:** Set up and synchronize an Ergo node.

## Keywords
indexing, node, rest, chain, strategy, querying, approach, datum, ergo, api](swagger.md, application, concept, party, explorer, communicate, instance, copy, blockchain, endpoint, block
