...
Source: docs/dev/stack/utilities/plutomonkey.md
...
## Keywords
ergoscript, contract, script, address, plutomonkey, https://wallet.plutomonkey.com/p2s/?source=, rust, code, snippet, p2s_addr_str, encoder, addressencoder::new(networkprefix::mainnet, addr, expr, addr.script().unwrap().proposition().unwrap, dbg!(&script, bool, eval_out_wo_ctx::<sigmaprop>(script.as_ref, assert!(res, link](https://github.com
...
```rust
    fn simplified_age_usd_bank_contract() {
        ...
        let encoder = AddressEncoder::new(NetworkPrefix::Mainnet);
        let addr = encoder.parse_address_from_str(p2s_addr_str).unwrap();
        let script: Rc = addr.script().unwrap().proposition().unwrap();
        dbg!(&script);
        let res: bool = eval_out_wo_ctx::(script.as_ref())
            .try_into()
            .unwrap();
        assert!(res);
    }
