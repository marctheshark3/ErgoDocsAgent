# P2S Playground - ErgoDocs
Source: [https://docs.ergoplatform.com/dev/stack/utilities/plutomonkey/](https://docs.ergoplatform.com/dev/stack/utilities/plutomonkey/)
Generated: 2025-05-11

## Summary
To compile any ErgoScript contract into a P2S (Pay-to-Script) address, you can use plutomonkey: After compiling, the resulting P2S address can be utilized within Rust, as demonstrated in the following code snippet: fn simplified_age_usd_bank_contract() {
        let p2s_addr_str = "";
        let encoder = AddressEncoder::new(NetworkPrefix::Mainnet);
        let addr = encoder.parse_address_from_str(p2s_addr_str).unwrap();
        let script: Rc<Expr> = addr.script().unwrap().proposition().unwrap();
        dbg!(&script);
        let res: bool = eval_out_wo_ctx::<SigmaProp>(script.as_ref())
            .try_into()
            .unwrap();
        assert!(res);
    }

See this link for full context This approach should also be applicable with JavaScript or TypeScript WASM (WebAssembly) bindings.

## Keywords
ergoscript, contract, script, address, plutomonkey, rust, code, snippet, p2s_addr_str, encoder, addressencoder::new(networkprefix::mainnet, addr, expr, addr.script().unwrap().proposition().unwrap, dbg!(&script, bool, eval_out_wo_ctx::<sigmaprop>(script.as_ref, assert!(res, link, context

## Content
## Compiling ErgoScript#
To compile any ErgoScript contract into a P2S (Pay-to-Script) address, you can use plutomonkey:
https://wallet.plutomonkey.com/p2s/?source=
After compiling, the resulting P2S address can be utilized within Rust, as demonstrated in the following code snippet:
fn simplified_age_usd_bank_contract() {
        let p2s_addr_str = "";
        let encoder = AddressEncoder::new(NetworkPrefix::Mainnet);
        let addr = encoder.parse_address_from_str(p2s_addr_str).unwrap();
        let script: Rc<Expr> = addr.script().unwrap().proposition().unwrap();
        dbg!(&script);
        let res: bool = eval_out_wo_ctx::<SigmaProp>(script.as_ref())
            .try_into()
            .unwrap();
        assert!(res);
    }

See this link for full context
This approach should also be applicable with JavaScript or TypeScript WASM (WebAssembly) bindings.
