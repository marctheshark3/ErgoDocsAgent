# Technical Breakdown - ErgoDocs
Source: [https://docs.ergoplatform.com/mining/algo-technical/](https://docs.ergoplatform.com/mining/algo-technical/)
Generated: 2025-05-11

## Summary
Autolykos is a proof-of-work (PoW) algorithm utilized in blockchain mining. It's based on the principles outlined in the Equihash paper, which itself is an exploration of the generalized birthday problem. This guide provides an in-depth explanation of the Autolykos algorithm by breaking down its pseudocode and underlying mechanics. In essence, miners are tasked with finding k (=32) elements from a set of N elements. The chosen elements should yield a sum that, when hashed, has a hash value lower than a target value.

## Keywords
autolykos, proof, work, algorithm, blockchain, mining, principle, equihash, paper, exploration, birthday, problem, guide, depth, explanation, pseudocode, mechanic, essence, miner, element
...
This index can be translated to values in base ten as it refers to numbers in [0, N). For instance, 0x2BFE8C0 = 46131392, 0x3E8C0A1 = 65585313, 0xC0A1BA = 12624314, and so...

### Line 8: sum of r elements given k#
Using the index generated in line 7, the miner retrieves the corresponding k (=32) r values from list R and sums these values. This might sound confusing but letâs break it down.
...
{0 | 46,131,392} â _dropMsb(H(46,131,392||h||M))_
{1 | 65,585,313} â _dropMsb(H(65,585,313||h||M))_
{2 | 12,624,314} â _dropMsb(H(12,624,314||h||M))_
{3 | 10,599,011} â _dropMsb(H(10,599,011||h||M))_
â¦
{31 | 8,830,952} â _dropMsb(H(8,830,952||h||M))_
...
The sum of the 32 r values is hashed using Algorithm 3, and if the output is below target b, the PoW is successful, m and nonce are returned to network nodes, and the miner is rewarded in ERG. If the sum hash is above the target, Lines 4 â 11 are repeated with a new nonce.
